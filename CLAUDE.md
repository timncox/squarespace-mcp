...
- **Endpoint drift is real.** Squarespace changes internal APIs under this server. 2026-07-08: injection values moved out of `/api/settings` (read from `/api/config/GetInjectionSettings`), and `SaveInjectionSettings` requires form-encoded `header`/`footer` (old `injectHeader` names were accepted-but-ignored — a silent no-op). 2026-07-19: `GET /api/settings` started serializing set-type fields as `{key: true}` maps while `PUT /api/settings` only accepts the array form the editor UI sends — so read-modify-write PUTs 400 (empty body) until sanitized; see `sanitizeSettingsForPut()` in `content-save/site.ts`. When a tool errors or silently no-ops, run `scripts/smoke.mjs` first, then capture ground truth with `scripts/capture-config-api.mjs <subdomain> </config/...>` — headless for read traffic, `--headful` to drive the panel by hand (e.g. click SAVE) while it records every request with full write payloads. Diff the UI's payload against the MCP client's. Note most config panels stage changes and only write on an explicit SAVE click; nothing fires on toggle/typing. Use the trial/sandbox site (`sphere-green-b2j7`) for write experiments, never a live client site.
...
| File | Purpose |
|------|---------|
| `src/mcp-server/index.ts` | MCP server entry — 134 tools across 16 modules |
...
- **`PUT /api/settings` rejects GET's own body**: GET returns `tutorialsCompleted` / `menuShortcuts` as `{key: true}` maps; PUT 400s (empty body) unless they're arrays (`["slug"]` / `[1]`), plus it never wants the server-managed flags (`bucketingSeedId`, `bundleEligible`, etc.). All settings PUTs must go through `sanitizeSettingsForPut()` (site.ts). Discovered 2026-07-19 via `capture-config-api.mjs` on the announcement-bar panel.
