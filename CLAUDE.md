...
| `src/services/conversation/execution.ts` | Task execution (single, batched, multi-site, blank_api) |
...
`ContentSpec` includes `contentStrategy`, `apiBlocks` (HTML blocks for blank_api), and `templateIndex` (for template selection).

### Content Save API Fast Paths

Several browser agent actions try the Content Save API first (~100-500ms) before falling back to UI automation. The API uses `PUT /api/page-sections/{pageId}/collection/{collectionId}` with a read-modify-write pattern: GET sections JSON → modify in memory → PUT back.
...
`executeBatchedPlan()` runs batches sequentially on the same page, emitting `agent_activity` and `agent_step` SSE events per batch. Each batch gets its own step budget.
...
- Run: `npm run test` (2 integration test files show as "failed" when no browser session — this is expected)
