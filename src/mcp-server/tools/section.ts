...
export function registerSectionTools(server: McpServer) {
  ...
  server.registerTool('sq_add_blank_section', {
    description: 'Add a new blank section to a Squarespace page.',
    inputSchema: {
    ...
    try {
      ...
      const result = await client.addBlankSection(ids.pageSectionsId, ids.collectionId);
      ...
      // Position parameter noted but not yet implemented — section is appended at end.
      // Reordering after add would require a separate moveSection call.
      const response: Record<string, any> = {
        success: true,
        sectionId: result.sectionId ?? null,
      };
      if (position !== undefined) {
        response.note = 'Section added at end. Use sq_move_section to reposition if needed.';
      }
