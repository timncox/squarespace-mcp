...
export class ContentSaveClient {
  ...
  /**
   * Hide a block on mobile by setting layout.mobile.visible = false.
   * Read-modify-write: GET → findBlock → set visible → PUT.
   *
   * Mobile grid: 8 columns. Confirmed field from page-sections-home.json.
   */
  async hideOnMobile(
    pageSectionsId: string,
    collectionId: string,
  ...
  /**
   * Show a block on mobile by setting layout.mobile.visible = true.
   * Read-modify-write: GET → findBlock → set visible → PUT.
   *
   * Mobile grid: 8 columns. Confirmed field from page-sections-home.json.
   */
  ...
  /**
   * Set a block's mobile layout — partial update of start, end, and/or visible.
   * Read-modify-write: GET → findBlock → apply → clamp → PUT.
   *
   * Mobile grid: 8 columns (x: 1–8). Omit any field to leave it unchanged.
   * Clamping: start.x < 1 shifts right; end.x > mobileColumns+1 shifts left;
   * start.y < 0 shifts down. Returns error if width or height is zero/negative.
   */
  ...
  /**
   * Move a block on the mobile grid by shifting its start/end coordinates.
   * Read-modify-write: GET → findBlock → shift → clamp → PUT.
   *
   * Mobile grid: 8 columns. Default step = block's own width (left/right) or height (up/down).
   */
  ...
  /**
   * Resize a block on the mobile grid by adjusting its end coordinates.
   * Read-modify-write: GET → findBlock → adjust end.x/end.y → clamp → PUT.
   *
   * Width: "smaller" shrinks by 2 cols, "larger" grows by 2, "full" spans all 8 mobile columns.
   * Height: "shorter" shrinks by 1 row, "taller" grows by 1 row.
   * Minimum size: 1 col wide, 1 row tall. Mobile grid: 8 columns.
   */
