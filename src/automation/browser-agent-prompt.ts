...
- Changes in block editors are often auto-saved when you click away or close the panel`,
  },
  {
    ...
    minHighConfToReduce: 4,
    removable: true,
    reduced: 'Block types: Text (dblclick to inline-edit), Button (dblclick to edit text/link/design: size/style/alignment/variant, delete via section edit mode), Image (dblclick for editor), Menu (dblclick to open structured editor with sections and items), Gallery, Embed/Code, Form. Never remove a block or section to edit content inside it.',
...
| \`editButtonBlock\` | Updates button label, URL, and design (size/style/alignment/variant) | API fails |
...
| editButtonBlock | searchText, newLabel?, url?, size?, style?, alignment?, variant? | **PREFERRED for editing buttons.** Finds a button by text, enters section edit mode, opens the button editor panel. Content tab: updates label (TEXT input) and/or URL (LINK picker). Design tab: sets size ("small"/"medium"/"large"), style ("primary"/"secondary"/"tertiary"), alignment ("left"/"center"/"right"), variant ("solid"/"outline"). Provide at least one param. searchText = current button text. For url, use full URLs like "https://example.com" or internal paths like "/projects". |
...
| addButtonBlock | label, url, size?, style?, alignment?, variant? | **Add a NEW button block with label and URL in one step.** Tries Content Save API first (~200ms), falls back to UI (ADD BLOCK → Button → editButtonBlock). Must already be in section edit mode (use enterSectionEditMode first). size: small/medium/large. style: primary/secondary/tertiary. alignment: left/center/right. variant: solid/outline. Use addButtonBlock to ADD new buttons, editButtonBlock to EDIT existing buttons. |
| createPage | title, slug?, template?, pageType? | Create a new page in the site. Navigates to Pages panel, clicks Add, fills in the title. pageType = "page" (default, blank page) or "blog" (creates a blog collection that supports posts). slug = optional URL slug (auto-generated from title if omitted). template = optional page template (default: Blank). |
...
22b. **When adding a NEW button, use addButtonBlock** instead of addBlockToSection("Button") + editButtonBlock. addButtonBlock handles adding the block and configuring label, URL, size, style, alignment, and variant in one step. Tries the Content Save API first for speed (~200ms), falls back to UI automation. Use addButtonBlock for NEW buttons, editButtonBlock for EXISTING buttons. You must be in section edit mode first (use enterSectionEditMode).
